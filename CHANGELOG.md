...
## [v5.0.0](https://github.com/stellar/js-xdr/compare/v4.0.0...v5.0.0)
...
- `Reader` accepts only byte-sized typed arrays (`Uint8Array` and its
  subclasses, including `Buffer`) and rejects other input with a `TypeError` at
  construction. v4's `XdrReader` coerced any `ArrayBuffer` view with
  `Buffer.from`, so a wider typed array such as `Uint16Array` was read
  element-by-element as truncated bytes
...
- `Reader` and `Writer` reject a non-integer or negative `maxDepth` with an
  ...
  ([#150](https://github.com/stellar/js-xdr/pull/150)).
- Encoding or decoding a non-empty `array` or `fixedArray` value throws
...
- Decoded byte values never share memory with the input. v4's
  `opaque`/`varOpaque`/`string` decoded to views of the input `Buffer`, which
  aliased the caller's input and could expose Node's shared `Buffer` pool;
  `Reader` now normalizes its input so `decode` returns independent copies
  ([#152](https://github.com/stellar/js-xdr/pull/152)).
- `Reader` treats input backed by a detached `ArrayBuffer` as empty, so
...
- `opaque`, `varOpaque`, and `string` reject a `Uint8Array` whose backing buffer
  ...
  prototype and reports `length === 0`, so it would otherwise reach the writer
  and fail with a native `TypeError`, escaping `validate` as a throw instead of
