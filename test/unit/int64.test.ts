...
import { int64 } from '../../src/index.js';
...
describe('int64', () => {
  describe('decode', () => {
    it('decodes big-endian signed bigints', () => {
      ...
      expect(
        schema.decode(bytes([127, 255, 255, 255, 255, 255, 255, 255]))
      ).toBe(MAX);
      expect(schema.decode(bytes([128, 0, 0, 0, 0, 0, 0, 0]))).toBe(MIN);
    });
  });

  describe('encode', () => {
    it('encodes big-endian signed bigints', () => {
      expect(toArray(schema.encode(1n))).toEqual([0, 0, 0, 0, 0, 0, 0, 1]);
      expect(toArray(schema.encode(-1n))).toEqual([
        255, 255, 255, 255, 255, 255, 255, 255
      ]);
      expect(toArray(schema.encode(MAX))).toEqual([
        127, 255, 255, 255, 255, 255, 255, 255
      ]);
      expect(toArray(schema.encode(MIN))).toEqual([128, 0, 0, 0, 0, 0, 0, 0]);
    });

    it('throws when out of range', () => {
      expect(() => schema.encode(MAX + 1n)).toThrow(/range/i);
      expect(() => schema.encode(MIN - 1n)).toThrow(/range/i);
    });
  ...
  it('round-trips boundary values', () => {
    for (const value of [0n, 1n, -1n, MAX, MIN]) {
