...
describe('Double.write', function() {
  ...
  it('encodes correctly', function() {
    ...
    expect(write(-0.0)).to.eql([0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00]);
    expect(write(1.0)).to.eql([0x3F,0xF0,0x00,0x00,0x00,0x00,0x00,0x00]); 
    expect(write(-1.0)).to.eql([0xBF,0xF0,0x00,0x00,0x00,0x00,0x00,0x00]);
    expect(write(NaN)).to.eql([0x7F,0xF8,0x00,0x00,0x00,0x00,0x00,0x00]);
  });
...
describe('Double.isValid', function() {
  ...
  it('returns true for numbers', function() {
    ...
    expect(Double.isValid(100000.00)).to.be.true;
