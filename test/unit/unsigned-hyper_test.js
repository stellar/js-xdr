...
describe('UnsignedHyper.read', function() {
  ...
  it('decodes correctly', function() {
    expect(read([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00])).to.eql(UnsignedHyper.fromString("0"));
    expect(read([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01])).to.eql(UnsignedHyper.fromString("1"));
    expect(read([0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF])).to.eql(UnsignedHyper.MAX_VALUE);
  });
...
describe('UnsignedHyper.write', function() {
  ...
  it('encodes correctly', function() {
    expect(write(UnsignedHyper.fromString("0"))).to.eql([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00]);
    expect(write(UnsignedHyper.fromString("1"))).to.eql([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01]);
    expect(write(UnsignedHyper.MAX_VALUE)).to.eql([0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF]);
  });
